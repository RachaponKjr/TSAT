...
export interface CMSCustomerProps {
    data: any
    id: string
    text_line_1: string
    text_line_2: string
...
const CustomerPage = () => {
    const [formData, setFormData] = useState<Partial<CMSCustomerProps>>({})

    const getCMS = useCallback(async () => {
        try {
            const { data: res } = await api.cms.getCMSCustomer()
    ...
    const handleChange = (key: keyof CMSCustomerProps) => (
    ...
    const handleSave = async () => {
        if (!formData) return
        const transformedData: Partial<CMSCustomerProps> = {}
        for (const key in formData) {
            const value = formData[key as keyof CMSCustomerProps] || ''
            transformedData[key as keyof CMSCustomerProps] = transformNewlinesToBr(value)
        }
        try {
            await api.cms.updateCMSCustomer(transformedData).then(() => {
    ...
    return (
        <div className='flex flex-col gap-4'>
            <h4 className='text-lg font-semibold text-center'>หน้าลูกค้า (Customer Page)</h4>
