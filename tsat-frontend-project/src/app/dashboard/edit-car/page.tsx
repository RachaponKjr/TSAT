...
const CarModelItem = ({
  ...
  const updateModel = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const cookie = await getCookie("access_token");
      const form = new FormData();
      form.append("name", editValue);
      form.append("image_model", editImage as File);
      await fetch(`http://150.95.26.51:3131/api/v1/car-model/${item.id}`, {
        method: "PUT",
        body: form,
        headers: {
          Authorization: `Bearer ${cookie}`,
        ...
        if (res.status === 200) {
          toast.success("สำเร็จ", { className: "!text-green-500" });
          setEditOpen(false);
          setEditValue("");
          setEditImage(null);
        }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 sm:p-6 bg-gray-50 border-b border-gray-200">
        ...
        <div className="flex gap-4">
          ...
          <Dialog open={editOpen} onOpenChange={setEditOpen}>
            <DialogTrigger>
              <button className="flex items-center gap-2 px-4 py-2 bg-yellow-300 hover:bg-yellow-400 cursor-pointer text-white rounded-lg transition-colors font-medium text-sm">
                <Edit size={14} />
                <span className="hidden sm:inline">เเก้ไข</span>
                <span className="sm:hidden">เเก้ไข</span>
              </button>
            </DialogTrigger>
            <DialogContent className="max-w-[95vw] sm:max-w-[600px] max-h-[90vh] overflow-y-auto p-4 sm:p-6 bg-white">
              <DialogTitle>เเก้ไข CarModel</DialogTitle>
              <form onSubmit={updateModel} className="flex flex-col gap-4">
                <InputWithLabel
                  ...
                  value={editValue}
                  name="editValue"
                  placeholder="กรุณา ใส่ชื่อใหม่"
                />
                <div className="flex flex-col gap-2">
                  <span className="text-sm font-medium text-gray-700">
                    รูปรถยนต์
                  </span>
                  <ImageBoxUpload value={item.image} onChange={setEditImage}/>
                </div>
                <button type="submit" className="h-[42px] rounded-lg bg-green-500 hover:bg-green-600 text-white cursor-pointer">
                  Update
                </button>
              </form>
            </DialogContent>
